...
export const MeetingUI = ({ chatClient, mode, queryParams = {} }: MeetingUIProps) => {
  ...
  const videoCodecOverride = (queryParams['video_encoder'] ||
    queryParams['video_codec']) as PreferredCodec | undefined;
  const fmtpOverride = queryParams['fmtp'] as string | undefined;
  const bitrateOverride = queryParams['bitrate'] as string | undefined;
  const videoDecoderOverride = queryParams['video_decoder'] as
    | PreferredCodec
    | undefined;
  const videoDecoderFmtpOverride = queryParams['video_decoder_fmtp'] as
    | string
    | undefined;
  const maxSimulcastLayers = queryParams['max_simulcast_layers'] as
    | string
    | undefined;
  ...
  const isSortingDisabled = queryParams['enableSorting'] === 'false';
  useEffect(() => {
    if (!call) return;
